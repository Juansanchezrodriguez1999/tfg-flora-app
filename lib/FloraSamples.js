...
localStorage.version(1).stores({
  samples:
    "id, No_Register, Date, *Authors, Group, Project, Location, Natural_Park, UTM, Latitude, Longitude, Lithology, Coverage, Altitude, Plot_Slope, Alt_Veg, Plot_Area, Plot_Orientation, Ecology, Community, Community_Year, *Community_Authors, Subcommunity, Subcommunity_Year, *Subcommunity_Authors, *Species, *Pictures, RemoteIdToUpdate, *PicturesToRemove",
});
...
const syncDoc = async (doc, username) => {
  ...
  if (remoteRegisNumbers.includes(doc._id)) {
    const error = new Error(`The registration number '${doc._id}' is already associated with another sample in the remote database. Change the corresponding sample and try again.`);
    error.name = doc._id
    throw error
  }
...
const FloraSamples = {
  ...
  syncAll: async (username) => {
    ...
    if (errors.length > 0) {
      ...
      errors.map((e) => {
        if (e.name === "Other") return new Promise(() => {throw new Error(e.message)})
        else badRegisNumbers.push(e.name)
      })
      return new Promise(() => {throw new Error(`The registration numbers '${badRegisNumbers.join("', '")}' are already associated with other samples in the remote database. Modify the corresponding samples and try again.`)})
    } else {
  ...
  removeRemoteSample: async (doc) => {
    ...
    return fetch(`/services/flora/app/api/${doc._id}`, { method: "DELETE" }).then(
      (response) => {
        if (response.ok) {
          return response;
        } else {
  ...
  getAllLocalRegisNumber: () => {
    return localStorage.samples.toArray().then((localSamples) => localSamples.map((doc) => doc._id));
  },
};
