...
const Time = {
  insertTime: () => {
    ...
    const newDateTime = new Date(
      `${year}/${month < 10 ? `0${month}` : `${month}`}/${date} ${hour}:${
  ...
  getAllTime: () => {
    return localStorage.DateTime.toArray().then((localSamples) =>
      localSamples.map((doc) => doc)
    );
  },
  getTime: () => {
    return localStorage.DateTime.toArray();
  },
  removeTime: (id) => {
